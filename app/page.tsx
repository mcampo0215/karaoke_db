      ...
      <div className="relative overflow-hidden bg-background w-full">
        ...
        <div className="relative max-w-7xl mx-auto px-4 py-20">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-4xl mx-auto">
            ...
            <div className="card p-8 text-center group hover:scale-105 transition-all">
              <div className="text-4xl font-black text-primary mb-3">{users.reduce((sum, user) => sum + user.songsCount, 0)}</div>
              ...
              >
                ...
                <div className="flex items-center gap-6 mb-8">
                  <div className="flex-1">
                    <h3 className="text-2xl text-center font-black mb-2 group-hover:text-primary transition-colors tracking-tight">
                      {user.name}
                    </h3>
                ...
                <button className="w-full btn btn-outline py-4 font-black text-lg relative overflow-hidden group/btn tracking-wide"
                  onClick={() => handleTogglePlaylist(user.id)}>
                  <span className="relative z-10">
                    {expandedUserId === user.id ? 'Hide Full Playlist' : 'View Full Playlist'}
                  </span>
                ...
                {expandedUserId === user.id && (
                  <div className="mt-6 border-t border-light pt-5">
                    ...
                    {loadingSongsUserId === user.id ? (
                      <p className="text-sm text-muted">Loading songs...</p>
                    ) : songsErrorByUser[user.id] ? (
                      <p className="text-sm text-error">{songsErrorByUser[user.id]}</p>
                    ) : extraSongs.length > 0 ? (
                      <div className="space-y-2">
                        {extraSongs.map((song, i) => (
                          <div
                            key={`${song}-${i}`}
                            className="text-sm p-3 rounded-lg bg-background border border-light"
                          >
                            {song}
                          </div>
                        ))}
